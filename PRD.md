...
- [ ] Audit all API client methods for error handling:
...
- [ ] Add `--verbose` flag handling to print debug info on errors
- [ ] Ensure all errors output valid JSON (never panic or print stack traces)
- [ ] Implement graceful handling of unexpected HTML responses (CAPTCHA, login redirects)

#### 8.2 Exit Codes
- [ ] Implement exit code system in `cmd/root.go`:
  - [ ] 0 for success
  - [ ] 1 for general error
  ...
  - [ ] 5 for rate limited
  - [ ] 6 for not found
- [ ] Map CLIError codes to exit codes
- [ ] Ensure all commands use consistent exit codes

#### 8.3 Input Validation
- [ ] Add ASIN format validation helper (10 alphanumeric characters)
- [ ] Add order ID format validation
- [ ] Add subscription ID format validation
- [ ] Add price range validation (min < max, both positive)
- [ ] Add quantity validation (positive integer, reasonable max)
...
- [ ] Ensure tokens/cookies never appear in logs
- [ ] Add request timing logging for performance debugging
...
- [ ] Add `ldflags` to embed version info at build time:
  ```
...
- [ ] Create separate repo: `homebrew-amazon-cli` (or `homebrew-tap`)
- [ ] Create Homebrew formula `amazon-cli.rb`:
  ```ruby
...
- [ ] Test installation: `brew tap michaelshimeles/tap && brew install amazon-cli`
...
- [ ] Add inline help text to all commands (Cobra Long descriptions)
...
- [ ] Review ClawdHub documentation for skills.md format
- [ ] Study existing ClawdHub skills for examples
- [ ] Identify required metadata fields

#### 10.2 Create skills.md
- [ ] Create `skills.md` file with proper structure:
...
- [ ] Document the `--confirm` requirement for purchase operations
...
- [ ] Document authentication expiry handling

#### 10.6 Final Review & Testing
- [ ] Validate skills.md against ClawdHub schema
- [ ] Test all example commands work as documented
- [ ] Verify all JSON schemas match actual output
- [ ] Submit to ClawdHub for review/publication
