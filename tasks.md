...
- [x] Create file `Makefile` with targets: `build` (go build -o amazon-cli .), `test` (go test -v ./...), `cover` (go test -coverprofile=coverage.out ./... - [ ] Create file `Makefile` with targets: `build` (go build -o amazon-cli .), `test` (go test -v ./...), `cover` (go test -coverprofile=coverage.out ./... && go tool cover -html=coverage.out), `lint` (golangci-lint run), `clean` (rm -f amazon-cli coverage.out).- [ ] Create file `Makefile` with targets: `build` (go build -o amazon-cli .), `test` (go test -v ./...), `cover` (go test -coverprofile=coverage.out ./... && go tool cover -html=coverage.out), `lint` (golangci-lint run), `clean` (rm -f amazon-cli coverage.out). go tool cover -html=coverage.out), `lint` (golangci-lint run), `clean` (rm -f amazon-cli coverage.out).

- [x] Run command `make test` to verify Makefile works and all tests pass.

- [x] Run command `make lint` to verify linting passes with zero issues.

- [x] Run command `make build` to verify build succeeds and produces amazon-cli binary.
...
- [x] Run command `go mod tidy` to clean up go.mod and go.sum files, removing any unused dependencies.

- [x] Run command `gofmt -w .` to format all Go files in the project.

- [x] Commit all changes with message "Complete amazon-cli implementation with comprehensive tests and documentation".
