...
func TestGetOrder(t *testing.T) {
	...
	_, err := client.GetOrder("123-4567890-1234567")
	if err == nil {
		t.Error("GetOrder() should return error for unimplemented feature")
	}

	if cliErr, ok := err.(*models.CLIError); ok {
		if cliErr.Code != models.ErrorCodeNotFound {
			t.Errorf("GetOrder() error code = %s, want %s", cliErr.Code, models.ErrorCodeNotFound)
		}
	}
}
